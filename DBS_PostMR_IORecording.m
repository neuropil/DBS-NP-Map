function varargout = DBS_PostMR_IORecording(varargin)
% DBS_POSTMR_IORECORDING MATLAB code for DBS_PostMR_IORecording.fig
%      DBS_POSTMR_IORECORDING, by itself, creates a new DBS_POSTMR_IORECORDING or raises the existing
%      singleton*.
%
%      H = DBS_POSTMR_IORECORDING returns the handle to a new DBS_POSTMR_IORECORDING or the handle to
%      the existing singleton*.
%
%      DBS_POSTMR_IORECORDING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DBS_POSTMR_IORECORDING.M with the given input arguments.
%
%      DBS_POSTMR_IORECORDING('Property','Value',...) creates a new DBS_POSTMR_IORECORDING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DBS_PostMR_IORecording_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DBS_PostMR_IORecording_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DBS_PostMR_IORecording

% Last Modified by GUIDE v2.5 15-Aug-2018 16:04:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @DBS_PostMR_IORecording_OpeningFcn, ...
    'gui_OutputFcn',  @DBS_PostMR_IORecording_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before DBS_PostMR_IORecording is made visible.
function DBS_PostMR_IORecording_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DBS_PostMR_IORecording (see VARARGIN)
...
handles.definput = {'s','f','0'};
handles.mri2use = 'native';


% handles.run.Enable = 'off';

% Choose default command line output for DBS_PostMR_IORecording
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes DBS_PostMR_IORecording wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DBS_PostMR_IORecording_OutputFcn(hObject, eventdata, handles)
...
if isequal(handles.step2.ForegroundColor,  [0 0 0]) && isequal(handles.step6.ForegroundColor ,[0 0 0])
    ...
    if strcmp(handles.mri2use,'mni')
        [ handles.output_args ] = ExtractDBSPolygon( ele_m_nii  , 1.3 , 80);
        handles.mriMatDat = load_nii(mni_nii);
    elseif strcmp(handles.mri2use,'native')
        [ handles.output_args ] = ExtractDBSPolygon( ele_n_nii  , 1.3 , 80);
        handles.mriMatDat = load_nii(mri_n_nii);
    end
    
    [handles.brainIm] = Convert2single(handles.mriMatDat);
    ...
    handles.sliceThick = handles.mriMatDat.original.hdr.dime.pixdim(4);
    if strcmp(handles.definput{2},'f')
        handles.size = 1;
    else
        handles.size = 0;
    end
    if strcmp(handles.definput{2},'s')
        handles.color = 1;
    else
        handles.color = 0;
    end
    [handles.x,handles.y,handles.z,handles.sz,handles.col] =...
        getNeuroGUIparams(handles.neuroDat, handles.output_args,...
        handles.color,handles.size,handles.sliceThick);
...
hold on
Plot3D_EleBoundary( handles.output_args );
hold on
...
[handles.x,handles.y,handles.z,handles.sz,handles.col] =...
    getNeuroGUIparams(handles.neuroDat, handles.output_args,handles.color,handles.size,handles.sliceThick);
makePlot(handles)
guidata(hObject, handles);
